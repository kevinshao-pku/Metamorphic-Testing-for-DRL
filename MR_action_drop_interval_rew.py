...
def play_game_session(model, num_episodes=3, seed=42, drop_interval=0):
    """
    Runs a game session, potentially dropping actions (replacing with NOOP) at a fixed interval.
    ...
    for i in range(num_episodes):
        ...
        while not terminated:
            step_counter += 1
            if drop_interval > 0 and step_counter % drop_interval == 0:
...
def main():
    parser = argparse.ArgumentParser(description="Metamorphic test for action drop robustness based on a fixed interval.")
    parser.add_argument("--model", type=str, required=True, choices=["dqn", "ppo"], help="Model to test (dqn or ppo)")
    parser.add_argument("--drop_interval", type=int, default=10, help="Interval for dropping an action (forcing NOOP). 0 to disable.")
    ...
    original_rewards = play_game_session(model, seed=args.seed, drop_interval=0)
    original_reward = sum(original_rewards)

    print(f"\nRunning mutated case (dropping actions every {args.drop_interval} steps)...")
    mutated_rewards = play_game_session(model, seed=args.seed, drop_interval=args.drop_interval)
