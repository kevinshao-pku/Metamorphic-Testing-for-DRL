...
def play_game_session(model, num_episodes=3, seed=42, noise_interval=0):
    """
    Runs a game session, potentially forcing random actions (action noise) at a fixed interval.
    ...
    for i in range(num_episodes):
        ...
        while not terminated:
            step_counter += 1
            # Inject noise at a fixed interval
            if noise_interval > 0 and step_counter % noise_interval == 0:
                # Replace the agent's action with a random action
...
def main():
    parser = argparse.ArgumentParser(description="Metamorphic test for action execution robustness using action noise at a fixed interval.")
    parser.add_argument("--model", type=str, required=True, choices=["dqn", "ppo"], help="Model to test (dqn or ppo)")
    parser.add_argument("--noise_interval", type=int, default=10, help="Interval for forcing a random action. 0 to disable.")
    ...
    print(f"\nRunning original case (no action noise)...")
    original_rewards = play_game_session(model, seed=args.seed, noise_interval=0)
    original_reward = sum(original_rewards)

    print(f"\nRunning mutated case (forcing random actions every {args.noise_interval} steps)...")
    mutated_rewards = play_game_session(model, seed=args.seed, noise_interval=args.noise_interval)
