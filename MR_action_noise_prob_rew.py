...
def play_game_session(model, num_episodes=3, seed=42, noise_probability=0.0):
    """
    Runs a game session, potentially forcing random actions (action noise).
    ...
    for i in range(num_episodes):
        ...
        while not terminated:
            # Inject noise with a certain probability
            if noise_probability > 0 and random.random() < noise_probability:
                # Replace the agent's action with a random action, wrapped in a list
...
def main():
    parser = argparse.ArgumentParser(description="Metamorphic test for action execution robustness using action noise.")
    parser.add_argument("--model", type=str, required=True, choices=["dqn", "ppo"], help="Model to test (dqn or ppo)")
    parser.add_argument("--noise_probability", type=float, default=0.1, help="Probability of forcing a random action.")
    ...
    original_rewards = play_game_session(model, seed=args.seed, noise_probability=0.0)
    original_reward = sum(original_rewards)

    print(f"\nRunning mutated case (forcing random actions with {args.noise_probability:.0%} probability)...")
    mutated_rewards = play_game_session(model, seed=args.seed, noise_probability=args.noise_probability)
