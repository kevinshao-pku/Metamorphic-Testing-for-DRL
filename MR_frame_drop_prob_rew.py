...
def play_game_session(model, num_episodes, seed, drop_probability=0.0):
    """
    Plays game sessions with probabilistic frame drop.
    A "dropped" frame is a transient error where the agent's observation of the newest frame
    is replaced by the second-newest frame.
    """
    ...
    for i in range(num_episodes):
        ...
        while not done:
            ...
            # CORE LOGIC CHANGE: Probabilistically drop a frame
            if drop_probability > 0 and np.random.rand() < drop_probability:
...
def main():
    parser = argparse.ArgumentParser(description="Metamorphic test for performance change with probabilistic frame drop.")
    ...
    parser.add_argument("--drop_probability", type=float, default=0.1,
                        help="Probability of dropping a frame.")
    ...
    print(f"\nRunning original case (no drop)...")
    original_rewards = play_game_session(model, num_episodes=args.num_episodes, seed=args.seed, drop_probability=0.0)
    original_reward_total = sum(original_rewards)

    print(f"\nRunning mutated case (drop probability: {args.drop_probability})...")
    ...
    print(f"Drop Probability (p):  {args.drop_probability:>8.2f}")
