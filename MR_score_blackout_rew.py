...
def play_game_session(model, num_episodes=3, seed=42, black_out_score=False, save_visualization=False, score_rows=10):
    """
    Runs a game session, optionally blacking out the score from observations.
    ...
    for i in range(num_episodes):
        ...
        while not terminated[0]:
            ...
            # Apply the score blackout mutation if enabled
            if black_out_score:
                # Make sure we have a mutable numpy array, not a LazyFrames object
                ...
                if save_visualization and not visualization_saved and step_counter >= 500:
                    ...
                    original_frame = obs_array[0, :, :, -1]
                    plt.imsave("original_frame.png", original_frame, cmap='gray')

                    # Create a mutated copy for visualization from the original frame
                    mutated_frame_viz = np.copy(original_frame)
                    mutated_frame_viz[-score_rows:, :] = 0  # Black out BOTTOM rows
                    plt.imsave("mutated_frame.png", mutated_frame_viz, cmap='gray')

                    print(
                        f"\nSaved visualization frames (bottom {score_rows} rows blacked out): 'original_frame.png' and 'mutated_frame.png'")
                    visualization_saved = True
                # --- End Visualization Logic ---

                # Apply the mutation for the agent's action.
                # The agent needs all 4 frames in the stack to be mutated.
                final_obs = np.copy(obs_array)
                final_obs[0, -score_rows:, :, :] = 0  # Black out BOTTOM rows
        ...
        case = "Mutated" if black_out_score else "Original"
...
def main():
    parser = argparse.ArgumentParser(description="Metamorphic test for performance change when score is hidden.")
    parser.add_argument("--model", type=str, required=True, choices=["dqn", "ppo"],
                        help="Model to test (dqn or ppo)")
    ...
    parser.add_argument("--save_frame_visualization", action="store_true",
                        help="Save a before/after image of the score blackout mutation.")
    parser.add_argument("--score_rows", type=int, default=10,
                        help="Number of bottom rows to black out for the mutation.")
    ...
    # Run original case
    print(f"\nRunning original case (score visible)...")
    original_rewards = play_game_session(model, seed=args.seed, num_episodes=args.num_episodes, black_out_score=False,
                                         score_rows=args.score_rows)
    original_reward_total = sum(original_rewards)

    # Run mutated case
    print(f"\nRunning mutated case (score blacked out)...")
    mutated_rewards = play_game_session(model, seed=args.seed, num_episodes=args.num_episodes, black_out_score=True,
                                        save_visualization=args.save_frame_visualization, score_rows=args.score_rows)
    ...
    print("\n" + "-" * 50)
    print("      SCORE INVARIANCE PERFORMANCE RESULTS")
